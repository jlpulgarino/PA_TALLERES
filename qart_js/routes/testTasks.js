...
router.get('/:id', function(req, res, next) {
    db.TestTask.findById(req.params.id).then(function(resp) {
...
router.get('/', function(req, res, next) {
    db.TestTask.findAll().then(function(resp) {
        return res.send(resp);
    }).catch(next);
});
...
router.delete('/:id', function(req, res, next) {
    ...
    db.TestTask.findById(req.params.id).then(function(departamentoEliminado) {
...
router.post('/', function(req, res, next) {
    ...
    var departamentoEmulado = {
        id: data.get('id'),
        sequence: data.get('sequence', 'sequence field is required'),
        testType: data.get('testType', 'testType field is required'),
        descr: data.get('descr', 'testType field is required'),
        objective: data.get('objective'),
        withVRT: data.get('withVRT', 'withVRT field is required'),
        StrategyId: data.get('StrategyId', 'StrategyId field is required')
    };

    db.TestTask.save(departamentoEmulado).then(function(departamentoActualizado) {
